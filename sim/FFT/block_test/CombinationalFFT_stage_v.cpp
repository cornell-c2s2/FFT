...
#include "obj_dir_CombinationalFFT_stage/VCombinationalFFT_stage.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_CombinationalFFT_stage/VCombinationalFFT_stage__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  } VCombinationalFFT_stage_t;

  // Exposed methods
  VCombinationalFFT_stage_t * create_model( const char * );
  void destroy_model( VCombinationalFFT_stage_t *);
  void comb_eval( VCombinationalFFT_stage_t * );
  void seq_eval( VCombinationalFFT_stage_t * );
  void assert_en( bool en );

  #if VLINETRACE
  void trace( VCombinationalFFT_stage_t *, char * );
  #endif
...
VCombinationalFFT_stage_t * create_model( const char *vcd_filename ) {

  VCombinationalFFT_stage_t * m;
  VCombinationalFFT_stage   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (VCombinationalFFT_stage_t *) malloc( sizeof(VCombinationalFFT_stage_t) );
  model = new VCombinationalFFT_stage();
...
void destroy_model( VCombinationalFFT_stage_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VCombinationalFFT_stage * model = (VCombinationalFFT_stage *) m->model;
...
void comb_eval( VCombinationalFFT_stage_t * m ) {
...
void seq_eval( VCombinationalFFT_stage_t * m ) {
...
#if VLINETRACE
void trace( VCombinationalFFT_stage_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_CombinationalFFT_stage__v );
