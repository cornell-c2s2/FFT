...
#include "obj_dir_CombinationalFFT/VCombinationalFFT.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_CombinationalFFT/VCombinationalFFT__Syms.h"
#include "svdpi.h"
#endif
...
extern "C" {
  ...
  #if VLINETRACE
  void trace( VCombinationalFFT_t *, char * );
  #endif
...
VCombinationalFFT_t * create_model( const char *vcd_filename ) {

  VCombinationalFFT_t * m;
  VCombinationalFFT   * model;

  Verilated::randReset( 0 );
  Verilated::randSeed( 0 );

  m     = (VCombinationalFFT_t *) malloc( sizeof(VCombinationalFFT_t) );
  model = new VCombinationalFFT();
...
void destroy_model( VCombinationalFFT_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VCombinationalFFT * model = (VCombinationalFFT *) m->model;
...
void comb_eval( VCombinationalFFT_t * m ) {
...
void seq_eval( VCombinationalFFT_t * m ) {
...
#if VLINETRACE
void trace( VCombinationalFFT_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_CombinationalFFT__v );
